...
export default async function handler(req, res) {
  ...
  // FIX: Accept both 'type' and 'name' parameter
  const type = req.query.type || req.query.name;
  const page = req.query.page || 1;

  if (!type) return res.status(400).json({ status: false, message: "Missing type parameter" });
